...
@Component({
  ...
  imports: [CommonModule, CodeInput, ReactiveFormsModule, ValidationMessage, RouterLink],
  ...
  submit() {
    ...
    if(this.form.valid && this.email && this.fullCode && this.fullCode.length == 6)
    {
      this.accountService.resetPassword(
        new ResetPasswordModel(this.email, this.fullCode, this.form.get('newPassword')?.value)
      ).subscribe({
        next: (response) => {
