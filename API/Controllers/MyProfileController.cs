...
namespace API.Controllers
{
	...
	public class MyProfileController : ApiCoreController
	{
		...
		public async Task<ActionResult<ApiResponse>> EditMyProfile(EditMyProfileDto model)
		{
			...
			if (isEmailChanged && await CheckEmailExistsAsync(model.Email))
				return BadRequest(new ApiResponse(400, message: $"An account has been registered with '{model.Email}'. Please try using another email address.", displayByDefault: true));

			if (!user.UserName.Equals(model.Name.ToLower()) && await CheckNameExistsAsync(model.Name))
				return BadRequest(new ApiResponse(400, message: $"An account has been registered with '{model.Name}'. Please try using another name (username).", displayByDefault: true));
			...
			if (isEmailChanged)
			{
				...
				try
				{
					...
					{
						RemoveJwtCookie();
						return Ok(new ApiResponse(200, title: "Email was changed.", message: "Your changes have been saved, and you've been logged out due to the email change. Please confirm your email address."));
					}
			...
			else
			{
				...
				return Ok(new ApiResponse(200, message: "Your changes have been saved successfully.", showWithToastr: true, displayByDefault: true, data: appUserDto));
			}
		}
		...
		public async Task<ActionResult<ApiResponse>> MfaEnable(MfaEnableDto model)
		{
			...
			if (!isValid)
				return BadRequest(new ApiResponse(400, message: "Invalid code, please try again", displayByDefault: true));

			var result = await UserManager.SetTwoFactorEnabledAsync(user, true);
			if (!result.Succeeded)
				return BadRequest(new ApiResponse(400, displayByDefault: true));
